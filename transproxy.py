        ...
        for key, values in self.headers:
            ...
            for value in values:
                ...
                if key == 'Content-Length':
                    self.sendHeader('Content-Length', len(self.postData))
                else:
...
class ProxyRequest(http.Request):
    ...
    def process(self):
      ...
      #  self.setHeader('client_mac', client_mac)
      #  self.setHeader('rpi_mac', rpi_mac)

        log.msg("Request IP : %s, Request MAC :  %s , RPI MAC: %s, Request URL: %s"  % (ip, client_mac , rpi_mac, url))
        ...
        postData += '&client_mac=%s&rpi_mac=%s' % (client_mac, rpi_mac)
